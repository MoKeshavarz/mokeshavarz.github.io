...
export function BookDetailView({ book, part }: { book: Book; part?: BookPart }) {
  ...
  return <><Header /><main id="main"><article className="book-detail container">
    <Link className="back-link" href={part ? bookUrl : "/library"}>{part ? `← ${book.title} overview` : "← Personal library"}</Link>
    ...
    <div className="book-notes">
      <aside>
        ...
        {parts.length ? <nav className="book-parts-list" aria-label="Book parts"><Link href={bookUrl} aria-current={!part ? "page" : undefined}>Overview</Link>{parts.map((item, index) => <Link key={item.slug} href={`${bookUrl}/${item.slug}`} aria-current={part?.slug === item.slug ? "page" : undefined}>{String(index + 1).padStart(2, "0")} · {item.title}</Link>)}</nav> : <BookRecord book={book} />}
      </aside>
    ...
    {part && <nav className="part-navigation" aria-label="Previous and next book part">
      {previousPart ? <Link href={`${bookUrl}/${previousPart.slug}`}>← <span>Previous part</span><strong>{previousPart.title}</strong></Link> : <span className="part-navigation-disabled">← <span>Previous part</span></span>}
      <Link className="all-parts-link" href={bookUrl}>All parts</Link>
      {nextPart ? <Link className="part-navigation-next" href={`${bookUrl}/${nextPart.slug}`}><span>Next part</span> →<strong>{nextPart.title}</strong></Link> : <span className="part-navigation-disabled part-navigation-next"><span>Next part</span> →</span>}
    </nav>}
  </article></main><Footer /></>;
}

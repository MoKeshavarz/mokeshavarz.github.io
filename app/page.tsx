  ...
  return (
    <main>
      <header className="site-header shell">
        <a className="brand" href="#top" aria-label="Back to top"><span className="brand-mark">{profile.initials}</span><span>{profile.name}</span></a>
        <nav aria-label="Main navigation"><a href="/experience">Experience</a><a href="/journal">Journal</a><a href="#reading">Reading</a></nav>
      ...
      <section className="hero shell" id="top">
        <div className="hero-copy">
          ...
          <div className="hero-actions"><a className="button primary" href="/experience">Explore my experience <span>→</span></a><a className="text-link" href={`mailto:${profile.email}`}>Send an email <span>↗</span></a></div>
      ...
      <section className="trust-strip"><div className="shell trust-inner"><p>// CURRENT_FOCUS</p>{currentFocus.map((focus, index) => <Fragment key={focus}><span>{focus}</span>{index < currentFocus.length - 1 && <i />}</Fragment>)}</div></section>
      ...
      <section className="section shell skills-section"><div className="section-heading compact"><div><p className="kicker">Toolkit</p><h2>How I help.</h2></div><p>Broad enough to see the system, focused enough to sweat the details.</p></div><div className="skill-cloud">{skills.map((skill, index) => <span key={skill}><b>{String(index + 1).padStart(2, "0")}</b>{skill}</span>)}</div></section>

      <section className="section journal-section" id="journal"><div className="shell">
      ...
      <section className="section shell beyond" id="activities">
      ...
      <section className="contact-section shell" id="contact"><p className="kicker">Have a thoughtful project?</p><h2>Let&apos;s make something<br /><em>worth using.</em></h2><a className="button primary light" href={`mailto:${profile.email}`}>Start a conversation <span>↗</span></a><p>{profile.email}</p></section>

      <footer className="footer shell"><p>© 2026 {profile.name}. Built with curiosity and care. <span className="ai-powered">AI Powered</span></p><div><a href="#top">Back to top ↑</a><a href="https://github.com/MoKeshavarz" target="_blank" rel="noreferrer">GitHub ↗</a><a href="https://www.linkedin.com/" target="_blank" rel="noreferrer">LinkedIn ↗</a></div></footer>
    </main>
  );
}

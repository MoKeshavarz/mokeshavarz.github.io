...
import { books, experiences, projects, writing } from "./content";
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  return [
    ...staticRoutes.map((route) => ({ url: `${base}${route}`, lastModified: new Date("2026-08-28"), changeFrequency: route === "" ? "weekly" as const : "monthly" as const, priority: route === "" ? 1 : .8 })),
    ...experiences.map((item) => ({ url: `${base}/experience/${item.slug}`, lastModified: new Date("2026-08-28"), changeFrequency: "yearly" as const, priority: .8 })),
    ...projects.map((item) => ({ url: `${base}/projects/${item.slug}`, lastModified: new Date("2026-08-28"), changeFrequency: "yearly" as const, priority: .8 })),
